...
// --- Rate rule conditions & calculation (subcollection)
...
function toContractStatus(v: unknown): ContractStatus {
  ...
  if (s === "active" || s === "expired" || s === "cancelled") return s;
  return "draft";
}

function toBillingCycle(v: unknown): BillingCycle {
  const s = String(v ?? "").toLowerCase();
  if (s === "weekly" || s === "per_trip") return s;
  return "monthly";
}
...
function toRuleType(v: unknown): RateRuleType {
  const s = String(v ?? "").toLowerCase();
  if (s === "extra_charge" || s === "penalty" || s === "discount") return s;
  return "base";
}
...
export async function getContract(id: string): Promise<ContractRecord | null> {
  ...
  return d ? toContractRecord(d) : null;
}
...
export async function listContracts(): Promise<ContractRecord[]> {
  ...
  return list.map((d) => toContractRecord(d));
}
...
export async function editContract(id: string, data: ContractEditInput): Promise<void> {
...
export async function removeManyContracts(items: { id: string }[]): Promise<void> {
  return deleteManyDocuments(COLLECTION, items);
}

// --- Rate rules (contracts/{contractId}/rateRules)
export async function listRateRules(contractId: string): Promise<RateRuleRecord[]> {
  const list = await getSubcollection<Record<string, unknown>>(COLLECTION, contractId, RATE_RULES_SUB);
  return list.map((d) => toRateRuleRecord(d)).sort((a, b) => a.priority - b.priority);
}

export async function getRateRule(
  contractId: string,
  ruleId: string
): Promise<RateRuleRecord | null> {
  ...
  return d ? toRateRuleRecord(d) : null;
}
...
export async function editRateRule(
  contractId: string,
  ruleId: string,
...
export async function removeRateRule(contractId: string, ruleId: string): Promise<void> {
