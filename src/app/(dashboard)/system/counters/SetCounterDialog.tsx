...
export default function SetCounterDialog({
  ...
  const hide = () => router.push("/system/counters");
  const onHide = () => {
    if (!saving) hide();
  };
  ...
  useEffect(() => {
    if (!visible) return;
    setError(null);
    sequenceService.list().then(setSequences).catch(() => setSequences([]));
    if (!counterId) {
      setSequenceId("");
      setPeriod("");
      setLastNumber("0");
  ...
  const save = async () => {
    ...
    try {
      ...
      if (counterId) {
        await counterService.edit(counterId, {
