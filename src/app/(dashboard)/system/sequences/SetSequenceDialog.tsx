...
import type { ResetPeriod } from "@/services/sequenceService";
import { RESET_PERIOD, statusToSelectOptions } from "@/constants/statusOptions";
...
export default function SetSequenceDialog({
  ...
  const hide = () => router.push("/system/sequences");
  const onHide = () => {
    if (!saving) hide();
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (sequenceId) {
        await sequenceService.edit(sequenceId, {
          entity: entity.trim(),
          prefix: prefix.trim(),
          digits: digitsNum,
    ...
    >
      {loading ? (
        ...
        <div className="flex flex-col gap-4 pt-2">
          ...
          <DpInput type="input" label="Entidad" name="entity" value={entity} onChange={setEntity} placeholder="trip" />
          <DpInput type="input" label="Prefijo" name="prefix" value={prefix} onChange={setPrefix} placeholder="TRIP" />
